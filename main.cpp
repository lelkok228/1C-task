...
int get_number_of_consecutive_cards(const std::vector<int>& stack,
  ...
  for (int i = static_cast<int>(stack.size()) - 2;
       i >= 0 && i > number_of_last_closed_card_ &&
       stack[i] == stack[i + 1] + 1;
       ++number_of_consecutive_cards, --i) {
  }
}
...
class StackOfCards {
 public:
  StackOfCards(std::string stack) {
    ...
    for (size_t i = 0; i < stack.size(); ++i) {
      if (stack[i] == ' ') {
        std::string card =
            stack.substr(beginning_of_new_card, i - beginning_of_new_card);
        if (card == "A") {
          stack_.push_back(14);
        } else if (card == "K") {
          stack_.push_back(13);
  ...
  int HowManyCardsCanBeTransferedTo(const StackOfCards& to) const {
    ...
    if (to.stack_.empty()) {
      return get_number_of_consecutive_cards(stack_,
                                             number_of_last_closed_card_);
    }

    bool can_remove_cards_starting_ith = true;
    for (int i = stack_.size() - 1; i >= 0 && i > number_of_last_closed_card_ &&
                                    can_remove_cards_starting_ith;
         --i) {
      if (i != stack_.size() - 1) {
  ...
  void TranserTo(StackOfCards& to, int number_of_transfered_card) {
    for (int i = stack_.size() - number_of_transfered_card; i < stack_.size();
         ++i) {
    ...
    number_of_last_closed_card_ = std::min(number_of_last_closed_card_,
                                           static_cast<int>(stack_.size()) - 2);
 ...
 private:
  void RemoveCardsIfPossible_() {
    ...
    int number_of_consecutive_cards =
        get_number_of_consecutive_cards(stack_, number_of_last_closed_card_);

    if (number_of_consecutive_cards == NUMBER_OF_CARD_VALUES) {
      for (int i = 0; i < number_of_consecutive_cards; ++i) {
        stack_.pop_back();
      }
      number_of_last_closed_card_ = std::min(
          number_of_last_closed_card_, static_cast<int>(stack_.size()) - 2);
    }
  }
...
class SpiderSolitaire {
 ...
 private:
  void RemoveAsManyCardsAsPossible_() {
    for (int number_of_moves = 0; number_of_moves < MAX_NUMBER_OF_MOVES_;
         ++number_of_moves) {
      ...
      int number_of_transfered_card =
          stacks_[from_id].HowManyCardsCanBeTransferedTo(stacks_[to_id]);
